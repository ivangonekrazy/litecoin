...
#if defined(NDEBUG)
# error "Logcoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Logcoin Signed Message:\n";
...
{
    // Logcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 2048 * COIN;

    // Subsidy is cut in half every 720 blocks, which will occur every 1 day
    // Should total up to 2,947,680 coin over 10 days.
    nSubsidy >>= (nHeight / 720); // Logcoin: 144 blocks in 1 day

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // Logcoin: 24 hours
static const int64 nTargetSpacing = 2 * 60; // Logcoin: 2 minute blocks
static const int64 nInterval = nTargetTimespan / nTargetSpacing; // retarget every 720 blocks
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Logcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Logcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Doctors pull chopstick from Chinese boy’s brain By Amy Graff on March 6, 2015 10:51 AM";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 2048 * COIN;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Logcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LogcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LogcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LogcoinMiner(CWallet *pwallet)
{
    printf("LogcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("logcoin-miner");
    ...
    try { loop {
        ...
        printf("Running LogcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LogcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LogcoinMiner, pwallet));
}
